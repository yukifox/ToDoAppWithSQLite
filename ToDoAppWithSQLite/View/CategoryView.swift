...
class CategoryView: UITableViewController {
    ...
    let categoryInstance = CategoryDataHelper.shared
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath, for orientation: SwipeActionsOrientation) -> [SwipeAction]? {
        ...
        let deleteAction = SwipeAction(style: .destructive, title: "Delete") { action, indexPath in
            if let category = self.categories[indexPath.row] {
                do {
                    let categoryData = CategoryBridge.toCategoryData(category: category)
                    try CategoryDataHelper.delete(item: categoryData)
            ...
            if let category = self.categories[indexPath.row] {
                ...
                let actionEdit = UIAlertAction(title: "Edit", style: .default, handler: {(action) in
                    if textField.text != category.name {
                        do {
                            try CategoryDataHelper.update(id: category.categoryTd!, with: textField.text)
                        } catch {
