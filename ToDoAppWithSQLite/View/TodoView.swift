...
class TodoView: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            do {
                try ItemDataHelper.delete(item: ItemBridge.toItemData(item: item))
            } catch {
...
extension TodoView: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        listItems = try! ItemDataHelper.findWithTitle(selectedCategoryId: selectedCategory?.categoryTd as! Int64, textInput: searchBar.text!)!
        isSearching = true
        print(listItems)
        tableView.reloadData()
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath, for orientation: SwipeActionsOrientation) -> [SwipeAction]? {
        ...
        if orientation == .right {
            ...
            let undoneAction = SwipeAction(style: .default, title: "Not Done", handler: {(action, indexPath) in
                ...
                do {
                    try ItemDataHelper.doneTask(with: item)
